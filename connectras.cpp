...
PIDParams anglePid[2] = {
    {0.2, 0.05, 0.2, 0, 0, 0},  // 左輪 (lKp, lKi, lKd, lTargetRPM, lintegralError, lLastError)
    {0.25, 0.05, 0.2, 0, 0, 0}  // 右輪 (rKp, rKi, rKd, rTargetRPM, rintegralError, rLastError)
};
...
void setup() {
  ...
  Serial.println("STATUS:ARDUINO_READY hahahahahahahahahahaha");
}
...
void loop() {
  ...
  if (stringComplete) {
    ...
    if (inputString.startsWith("AZ:")) {
      ...
      Serial.println("STATUS:RECEIVED_AZ:SUCCESS hahahahahahahahahahaha: " + inputString); 
    }
    else {
      Serial.println("STATUS:RECEIVED_AZ:FAIL hahahahahahahahahahaha: " + inputString); 
    }
    inputString = "";
  ...
  if (currentTime - prevTime >= 100) {
    ...
    float totalAngleError = targetAzimuth - currentHeading;
    ...
    float min_ratio = 0.25;         // 最低保留 25% 的控制力道，避免低速卡死
    float start_decel_angle = 45.0; // 剩餘 45 度以內開始平滑減速
    
    // 計算歸一化誤差比例
    float x = abs(totalAngleError) / start_decel_angle;
    if (x > 1.0) x = 1.0; // 超過 45 度就維持 1.0 原速
    ...
    anglePid[1].iError = constrain(anglePid[1].iError, -100, 100); // 限制積分防止暴衝
    ...
    anglePid[0].iError = constrain(anglePid[0].iError, -100, 100);
    ...
    leftPower  = ((5 + (anglePid[0].pError * anglePid[0].kp) + (anglePid[0].iError * anglePid[0].ki) + (anglePid[0].dError * anglePid[0].kd))) * velocity_factor;
